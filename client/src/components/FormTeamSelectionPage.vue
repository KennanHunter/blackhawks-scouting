    ...
    <FormPage title="Team Selection" ref="page">
        ...
        >
            <input
                ...
                type="text"
                v-model="eventKey"
                @keyup.enter="loadTBAData"
            />
            <button @click="loadTBAData">Load</button>
        </FormGroup>
        <FormGroup :label-type="LabelType.PlainText" name="Teams Loaded">{{
            teamsLoadStatus
        }}</FormGroup>
        <FormGroup :label-type="LabelType.PlainText" name="Matches Loaded">{{
            matchesLoadStatus
        }}</FormGroup>
        <FormGroup
            :label-type="LabelType.LabelTag"
            id="match-level-input"
            name="Match Level"
        >
            <select
                id="match-level-input"
...
const currentMatch = $computed(() => {
    ...
    const matchList = matches.filter(
        (match: unknown) =>
            get(match, "comp_level") === matchLevelCodes[matchLevel]
    );
    ...
    const getNumber = (matchObj: unknown) =>
        get(matchObj, "match_number") * 10 + get(matchObj, "set_number");

    matchList.sort((first: unknown, second: unknown) =>
        Math.sign(getNumber(first) - getNumber(second))
    );
...
const teamsList = $computed(() => {
    ...
    for (const color of ["Red", "Blue"]) {
        ...
        const teamKeys: string[] = get(
            currentMatch,
            `alliances.${color.toLowerCase()}.team_keys`
        );
...
const teamData = $computed(() =>
    teamsList[selectedTeam] ? Object.values(teamsList[selectedTeam]).join() : ""
);
...
function loadTBAData() {
    tba.load(eventKey, "teams").then((value) =>
        updateStatus($$(teamsLoadStatus), $$(teams), value)
    );
    tba.load(eventKey, "matches").then((value) =>
        updateStatus($$(matchesLoadStatus), $$(matches), value)
    );
}
</script>
