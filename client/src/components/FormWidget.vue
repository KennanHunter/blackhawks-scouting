<template>
    <FormGroup
        v-if="info"
        :id="id"
        :label-type="data.noLabel ? LabelType.None : info.label"
        v-bind="mappedProps"
    >
...
import { LabelType, WidgetData }from "@scout/shared";
...
const info = {
    ...
    multicheckbox: {
        class: WidgetMultiCheckbox,
        label: LabelType.PlainText,
        required: ["name", "options"],
    },
    picture: {
        class: WidgetPicture,
...
for (const i of info.required)
    if (!has(props.data, i))
        throw new Error(
            `Required attribute "${i}" was not specified in widget ${widgetName} (type ${widgetType})`
        );

// Props to pass from the widget data to the sub-components
const mappedProps = pick(props.data, [
    "name",
    "align",
    "row",
    "col",
...
function validate() {
    ...
    if (typeof widgetVal === "string" || Array.isArray(widgetVal))
        valueToValidate = widgetVal.length;
    ...
    const rangeValidationFuncs = new Map<
        string,
        (n: number, vals: number[]) => boolean
    >([
        [
            "inRange",
            (n: number, val: number[]) =>
                n >= Math.min(...val) && n <= Math.max(...val),
        ],
        [
            "outOfRange",
            (n: number, val: number[]) =>
