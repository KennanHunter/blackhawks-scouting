<template>
    <div v-show="show">
        ...
        <img
            v-if="config.data.logo"
            :src="absoluteLogoPath"
            alt="Cannot load logo file"
            class="center"
        />
...
const absoluteLogoPath = $computed(
    () => `${import.meta.env.BASE_URL}assets/${config.data.logo}`
);
...
defineExpose({
    title: props.title,
    setShown: (value: boolean) => (show = value),
});
</script>
