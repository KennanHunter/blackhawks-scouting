    ...
    <table class="inspector-table">
        <thead>
            <tr>
                ...
                <th v-for="[i, value] of data.header.entries()" :key="i">
                    {{ value }}
                </th>
            </tr>
        </thead>
        <tbody>
            <label
                v-for="[i, record] of data.values.entries()"
                :key="i"
                :class="{ select: selections.has(i) }"
            >
                <td>
                    <input
                        type="checkbox"
                ...
                <td v-for="[j, value] of record.entries()" :key="j">
                    {{ value }}
                </td>
            </label>
        </tbody>
...
watchEffect(() => {
    ...
    topCheckbox =
        selections.size > 0 && selections.size === props.data.values.length;
});

// Updates the selections to select all/none of the records based on the state of the top checkbox.
const updateSelections = () =>
    (selections = new Set(topCheckbox ? range(props.data.values.length) : []));
